...
export function CallRoom({
  ...
  const [clientRatingByLawyer, setClientRatingByLawyer] = useState<number>(0);
  const [clientCommentByLawyer, setClientCommentByLawyer] = useState("");
        ...
        <div className="space-y-4">
          ...
          {lawyerName && (
            <p className="text-sm text-slate-700">
              Profissional: <strong>{lawyerName}</strong>
              {lawyerOab ? ` · OAB ${lawyerOab}` : ""}
            </p>
          )}
          {error && (
        ...
        {role === "lawyer" && (
          <div>
            ...
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  ...
                  type="button"
                  className={`h-9 w-9 rounded-lg border text-sm ${
                    clientRatingByLawyer >= n
                      ? "border-amber-400 bg-amber-100 text-amber-800"
                      : "border-slate-200 bg-white text-slate-500"
                  }`}
                  onClick={() => setClientRatingByLawyer(n)}
                >
                  {n}★
                </button>
              ))}
            ...
            <textarea
              ...
              value={clientCommentByLawyer}
              onChange={(e) => setClientCommentByLawyer(e.target.value)}
            />
            <button
              type="button"
              disabled={pending}
              ...
              onClick={() =>
                start(async () => {
                  ...
                  const res = await rateClientAction(requestId, {
                    rating: clientRatingByLawyer || null,
                    comment: clientCommentByLawyer || null,
                  });
  ...
  return (
    <div>
      {(lawyerName || lawyerOab) && (
        <p className="mb-3 text-sm text-slate-700">
          Advogado: <strong>{lawyerName}</strong>
          {lawyerOab ? ` · OAB ${lawyerOab}` : ""}
        </p>
      )}
      ...
      {canComplete ? (
        ...
        <p className="text-sm text-slate-500">
          Aguarde o advogado finalizar, ou o timer de 30 min.
        </p>
      )}
    </div>
  );
