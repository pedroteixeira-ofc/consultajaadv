...
export function AdminPanel({
  ...
  const [tab, setTab] = useState<
    "lawyers" | "clients" | "review" | "ratings" | "payouts"
  >("lawyers");
  const [error, setError] = useState<string | null>(null);
  const [pending, start] = useTransition();

  const tabs = [
    ["lawyers", "Profissionais"],
    ["clients", "Clientes"],
  ...
  return (
    <div>
      ...
      {tab === "lawyers" && (
        <Card>
          <h2 className="mb-3 font-semibold">Advogados — OAB</h2>
              ...
              >
                <div>
                  <p className="font-medium">
                    {p.full_name}{" "}
                    <span className="text-slate-500">· {p.oab ?? "sem OAB"}</span>
                  </p>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={() =>
                      start(async () => {
                        setError(null);
                        const r = await setLawyerVerificationAction(
                          p.id,
                          "approved"
                        );
              ...
              {reviewQueue.map((r) => (
                ...
                >
                  <div>
                    ...
                    · {r.lawyer_name} ← {r.client_name} ·{" "}
                    {formatBRL(r.lawyer_cut_cents)}
                    <p className="text-xs text-slate-500">
            ...
            <ul className="divide-y divide-slate-100 text-sm">
              {ratings.map((r) => (
                <li key={r.id} className="py-3">
                  ...
                  <p className="text-xs text-slate-600">
                    Psic→Cliente:{" "}
                    {r.lawyer_rating_of_client
                      ? `${r.lawyer_rating_of_client}★`
                      : "—"}
                    {r.lawyer_rating_comment
                      ? ` “${r.lawyer_rating_comment}”`
                      : ""}
                  </p>
                </li>
        ...
        <Card>
          ...
          {owed.length === 0 ? (
            ...
            <ul className="mb-4 divide-y divide-slate-100 text-sm">
              {owed.map((o) => (
                <li key={o.lawyerId} className="py-2">
