  ...
  return (
    <>
      ...
      {payRequestId && (
        <Card className="mb-4 border-teal-200">
          ...
          <p className="mb-3 text-sm text-slate-600">
            ...
            · {formatBRL(payAmountCents ?? DEFAULT_PRICES.priceCents)}. Confirme para simular o
            webhook.
          </p>
          <button
            type="button"
      ...
      <Card>
        ...
        <p className="mb-3 text-sm text-slate-600">
          Especialidade fica visível na fila; o assunto só após o advogado
          aceitar. Anonimato é opcional.
        </p>
        {error && (
        ...
        <form
          ...
          action={(fd) =>
            start(async () => {
              setError(null);
              const r = await createConsultationAction(fd);
              if (r && !r.ok) setError(r.error);
              else router.refresh();
            })
          }
            ...
            {recent.map((r) => (
              <li key={r.id} className="flex justify-between gap-2 py-2">
                <span>
                  ...
                  · {r.specialty} ·{" "}
                  {r.is_anonymous ? "anônimo" : "identificado"} · {r.status}
