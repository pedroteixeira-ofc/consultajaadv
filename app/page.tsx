...
export default async function HomePage() {
  ...
  const monthly = formatBRL(db.platform_settings.monthly_fee_cents);
      ...
      <main className="mx-auto max-w-5xl px-4 pb-16 pt-8">
        <div className="max-w-2xl">
          ...
          <p className="mb-8 text-lg text-slate-600">
            Sessões de {minutes} minutos. Você escolhe se a requisição é
            identificada ou anônima. Advogados ficam online e aceitam a fila em
            tempo real.
          </p>
        ...
        <section className="mt-14 grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="text-sm text-slate-600">
              Conta + pedido anônimo opcional. Sessão de {minutes} minutos por{" "}
              {price}. Especialidade na fila; assunto só após aceite.
            </p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="mb-2 font-semibold text-slate-900">Advogados</h2>
            <p className="text-sm text-slate-600">
              Cadastro (OAB), mensalidade {monthly} (= e-mails) e status
              online. Aceitam a fila pendente. Pix só após sessão concluída.
            </p>
          </div>
        </section>
      </main>
