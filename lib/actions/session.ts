...
/**
 * Encerrar sessão (lawyer / timer / admin).
...
export async function completeSessionUiAction(
  ...
  if (session.role === "client") {
    ...
    if (elapsed < 30 * 60 * 1000 && req.status !== "completed") {
      // allow client to open post-session UI only after complete by lawyer/timer
    ...
    if (req.status !== "completed") {
      if (elapsed < 30 * 60 * 1000) {
        return {
          ok: false,
          error: "Aguarde o advogado encerrar, ou o timer de 30 min.",
        };
      }
...
export async function confirmAttendanceAction(
  ...
  const prep = await mutateStore((db) => {
    ...
    req.client_rating_of_lawyer = rating;
  ...
  const payout = await enqueueLawyerPayout({
    lawyerId: (prep as { lawyerId: string }).lawyerId,
...
/** Advogado avalia o cliente (opcional) após sessão completed. */
export async function rateClientAction(
  ...
  if (!session || session.role !== "lawyer") {
    return { ok: false, error: "Somente o advogado pode avaliar o cliente" };
  }
  const rating = opts.rating != null ? clampRating(opts.rating) : null;
  const comment = opts.comment?.trim() || null;
  ...
  return mutateStore((db) => {
    ...
    if (!req) return { ok: false as const, error: "Pedido não encontrado" };
    if (req.lawyer_id !== session.sub) {
      return { ok: false as const, error: "Não é o advogado desta sessão" };
    }
    if (req.status !== "completed") {
      return { ok: false as const, error: "Sessão ainda não encerrada" };
    }
    const now = nowIso();
    req.lawyer_rating_of_client = rating;
    req.lawyer_rating_comment = comment;
    req.updated_at = now;
    return { ok: true as const };
  });
}
